...
from pathway_identify import read_pathway
...
pathway_dict = read_pathway(path=f"../data/pathway/{dataset}/all.txt")

positive_genes = pd.read_csv(f"../data/{organism}_{dataset}_cellnet/valid_data/all_tf.csv").iloc[:, 0].values.tolist()
# total_nums = len(positive_genes)
# positive_genes = positive_genes[0:int(total_nums/3)]

train_tfs = pd.read_csv(f"../data/{organism}_{dataset}_cellnet/origin_net.csv").iloc[:, 0].values.tolist()
...
positive_pathways = pd.read_csv(f"../data/pathway/{dataset}/positive_pathways.csv",sep='\t').iloc[:, 0].values
